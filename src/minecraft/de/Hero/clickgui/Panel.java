...
public class Panel {
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		if(ClickGui.design.getCurrentOptionIndex() == 1){
		...
		}else if(ClickGui.design.getCurrentOptionIndex() == 0){
			Gui.drawRect(x + 4,			y + 2, x + 4, 		y + height - 2, 0xffaaaaaa);
			Gui.drawRect(x - 4 + width, y + 2, x - 4 + width, y + height - 2, 0xffaaaaaa);
			FontUtil.drawTotalCenteredStringWithShadow(title, x + width / 2, y + height / 2, 0xffefefef);
		}
		
		if (this.extended && !Elements.isEmpty()) {
			int startY = y + height;
			int epanelcolor = ClickGui.design.getCurrentOptionIndex() == 1 ? 0xff232323 : ClickGui.design.getCurrentOptionIndex() == 0 ? 0xbb151515 : 0;
			for (ModuleButton et : Elements) {
				if(ClickGui.design.getCurrentOptionIndex() == 1){
					Gui.drawRect(x - 2, startY, x + width, startY + (int)et.height + 1, outlineColor);
				}
				Gui.drawRect(x, 	startY, x + width, startY + (int)et.height + 1, epanelcolor);
				et.x = x + 2;
				et.y = startY;
				et.width = width - 4;
