...
public class ModuleButton {
	...
	public ModuleButton(Module imod, Panel pl) {
		 ...
		 */
		if (Vodka.INSTANCE.SETTING_MANAGER.getSettingsForModule(imod) != null)
			for (Setting s : Vodka.INSTANCE.SETTING_MANAGER.getSettingsForModule(imod)) {
				if (s.isBoolean()) {
	...
	public boolean mouseClicked(int mouseX, int mouseY, int mouseButton) {
		...
		if (mouseButton == 0) {
			...
			if (menuelements != null && menuelements.size() > 0) {
				...
				Vodka.INSTANCE.clickgui.closeAllSettings();
				this.extended = b;
	...
	public boolean keyTyped(char typedChar, int keyCode) throws IOException {
		...
		if (listening) {
			if (keyCode != Keyboard.KEY_ESCAPE) {
				PlayerUtils.tellPlayer("Bound '" + mod.getName() + "'" + " to '" + Keyboard.getKeyName(keyCode) + "'");
				mod.setKeyCode(keyCode);
			} else {
				PlayerUtils.tellPlayer("Unbound '" + mod.getName() + "'");
