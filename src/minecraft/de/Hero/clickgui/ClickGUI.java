...
public class ClickGUI extends GuiScreen {
	 ...
	 */
	public ClickGUI() {
		setmgr = Vodka.INSTANCE.SETTING_MANAGER;
		...
		for (Category c : Category.values()) {
			...
			ClickGUI.panels.add(new Panel(title, (int)px, (int)py, (int)pwidth, (int)pheight, false, this) {
						@Override
						public void setup() {
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		if(mb != null){
			...
			FontUtil.drawTotalCenteredStringWithShadow("Press 'ESCAPE' to unbind " + mb.mod.getName() + (mb.mod.getKeyCode() > -1 ? " (" + Keyboard.getKeyName(mb.mod.getKeyCode())+ ")" : ""), 0, 0, 0xffffffff);
