...
/**
 * Created by Dominic | Pr0T3ST on 07.05.2017.
 */
public class ClientPhysic {
	...
	public static void doRender(Entity par1Entity, double x, double y, double z, float par8, float par9) {
		...
		if (itemstack.getItem() != null) {
			...
			if ((item != null) && (!Double.isNaN(item.getAge())) && (!Double.isNaN(item.getAir()))
					&& (!Double.isNaN(item.getEntityId())) && (item.getPosition() != null)) {
				if (item.onGround) {
			...
			mc.getRenderManager().renderEngine.bindTexture(TextureMap.locationBlocksTexture);
			if (flag) {
				mc.getRenderManager().renderEngine.getTexture(TextureMap.locationBlocksTexture).getGlTextureId();
			}
		}
	}
