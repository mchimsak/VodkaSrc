...
import me.lc.vodka.event.events.EventInsideBlock;
...
public abstract class EntityPlayer extends EntityLivingBase
{
    ...
    public float getToolDigEfficiency(Block p_180471_1_)
    {
        ...
        if (Vodka.INSTANCE.MODULE_MANAGER.getModule("FovChanger").isToggled()) {
            f = 10.0F + (float) FovChanger.value.getCurrentValue() * 20.0F;
        }
        return f;
    }
    ...
    {
        EventInsideBlock event = new EventInsideBlock(Event.Type.PRE);
        event.call();
        return !this.sleeping && super.isEntityInsideOpaqueBlock() && (!event.cancel) ;
    }
